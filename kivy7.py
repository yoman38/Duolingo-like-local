...
class CourseCreation(BoxLayout):
    def __init__(self, preview_window, **kwargs):
        ...
        # Dropdown for bubble type
        self.bubble_type = TextInput(hint_text='Enter bubble type (Main or Course)', size_hint_y=0.2)
        self.main_bubble_dropdown = TextInput(hint_text='Enter associated Main Bubble (if any)', size_hint_y=0.2)
    ...
    def save_course(self, instance):
        ...
        main_bubbles = [course['title'] for course in load_courses_from_file() if course['type'] == 'Main']
        self.main_bubble_dropdown.values = main_bubbles
    ...
    def on_touch_down(self, touch):
        for course in load_courses_from_file():
            ...
            if ((touch.x - x) ** 2 + (touch.y - y) ** 2) <= 50**2:
                if course.get('type') == 'Main':  # Use the get method to safely access the 'type' key

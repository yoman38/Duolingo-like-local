from PySide6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, QComboBox,
                               ...
                               QSpinBox, QColorDialog, QCheckBox, QTabWidget, QScrollArea, QGraphicsView, QGraphicsScene, QGraphicsEllipseItem, QGraphicsTextItem, QTextBrowser, QGraphicsLineItem, QFileDialog, QDialog, QGraphicsPixmapItem)
...
class MixedContentWidget(QWidget):
...
sys.stdout = Logger("your_log_file.txt")
sys.stderr = Logger("your_error_file.txt")

class Bubble:
    def __init__(self, title, content, x=0, y=0, color=Qt.blue, is_holder=False, youtube_link=None, youtube_start_time=None, youtube_end_time=None, pdf_path=None):
        self.title = title
    ...
    def from_dict(cls, bubble_dict):
        ...
        bubble.children = [cls.from_dict(child_dict) for child_dict in bubble_dict.get('children', [])]
        return bubble
...
class CourseCreation(QWidget):
    ...
    def save_course(self):
        ...
        new_bubble = Bubble(title, content, x=x, y=y, color=color, is_holder=is_holder, youtube_link=self.youtube_link_input.text(), youtube_start_time=self.start_time_input.text(), youtube_end_time=self.end_time_input.text(),pdf_path=self.pdf_path if hasattr(self, 'pdf_path') else None) 
    ...
    def show_bubble_content(self, bubble):
        ...
        # Create MixedContentWidget and add to the dialog
        mixed_content_widget = MixedContentWidget()
        dialog_layout.addWidget(mixed_content_widget)

        # Parse the HTML content and set in the MixedContentWidget
        content_html = bubble.content
        
        # Logic to extract text, YouTube URLs, and PDF references from content_html
        # For now, just a simple example, you can make it more robust
        video_id_match = re.search(r'embed/([A-Za-z0-9_\-]+)', content_html)
        if video_id_match:
            video_id = video_id_match.group(1)
            mixed_content_widget.set_youtube_video(video_id)
            # Remove the video iframe from the HTML content
            content_html = re.sub(r'<iframe.*?iframe>', '', content_html)

        # Set text
        mixed_content_widget.set_text(content_html)

        if bubble.pdf_path:
            mixed_content_widget.set_pdf(bubble.pdf_path)

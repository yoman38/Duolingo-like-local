...
class CourseCreation(QWidget):
    ...
    def insert_youtube_video(self):
        ...
        embed_code = f'<iframe width="560" height="315" src="https://www.youtube.com/embed/{video_id}" frameborder="0"></iframe>'
        self.content_input.insertHtml(embed_code)
    ...
    def upload_pdf(self):
        ...
        if filePath:
            print(f"PDF file path: {filePath}")  # Debug
            self.pdf_path = filePath
            self.display_pdf_as_image(filePath)

    def display_pdf_as_image(self, pdf_path):
        # Clear existing items from the scene
        self.scene.clear()
        print(f"Trying to display PDF from path: {pdf_path}")  # Debug

        try:
            # Open the PDF file
            pdf_document = fitz.open(pdf_path)
            print(f"PDF has {len(pdf_document)} pages.")  # Debug
            ...
            for page_number in range(len(pdf_document)):
                ...
                print(f"Added page {page_number+1} to scene.")  # Debug
    ...
    def show_bubble_content(self, bubble):
        ...
        if bubble.youtube_link:
            ...
            content += f'<iframe width="560" height="315" src="https://www.youtube.com/embed/{video_id}" frameborder="0"></iframe>'

        # Add PDF if available
        if bubble.pdf_path:
            content += f'<embed src="{bubble.pdf_path}" type="application/pdf" width="100%" height="600px" />'

        # Close HTML tags
        content += "</body></html>"

        # Show content in a popup
        self.show_content_popup(content)

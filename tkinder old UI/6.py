...
def save_topic():
    ...
    if title and content:
        topics_content[title] = content
        bubble_colors[title] = color
        bubble_positions[title] = (x, y)
...
def create_bubble(canvas, title, x, y):
    color = bubble_colors.get(title, "light blue")
    canvas.create_oval(x-20, y-20, x+20, y+20, fill=color, tags=title)
    canvas.create_text(x, y, text=title, tags=title)
    canvas.tag_bind(title, '<Button-1>', lambda e, t=title: activate_bubble(t))

def activate_bubble(title):
    content = topics_content.get(title, "")
    popup = Toplevel(root)
    popup.title(title)
...
def display_learning_path():
    ...
    for title, (x, y) in bubble_positions.items():
        create_bubble(learning_canvas, title, x, y)
    link_nearby_bubbles(learning_canvas)

def link_nearby_bubbles(canvas):
    for title1, pos1 in bubble_positions.items():
        for title2, pos2 in bubble_positions.items():
            if title1 != title2:

...
def render_preview(event=None):
    ...
    type_ = bubble_type.get().lower()
    
    preview_canvas.delete("bubble")
    
    # Render courses from the courses dictionary
    ...
    # Render the current course being created
    if type_ == 'main':
        preview_canvas.create_oval(x-30, y-30, x+30, y+30, fill=color, tags="bubble")
    else:

...
def render_preview(event=None):
    ...
    preview_canvas.tag_bind(title, "<Button-1>", lambda e, title=title, details=courses[title]: display_course_content(title, details))
...
def save_topic():
    ...
    if title and content:
        courses[title] = {'content': content, 'x': x, 'y': y, 'color': color, 'type': 'course'}  # Assuming all new courses are of type 'course'
        save_courses()
...
def display_course_content(title, details):
    ...
    # Create a scrollable text widget with a fixed width
    ...
    # Insert the course content into the text widget

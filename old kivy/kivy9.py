...
class TextCustomizationToolbar(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Placeholder for title icon
        title_icon = Button(text="T", size_hint_x=None, width=40)
        title_icon.bind(on_press=self.show_title_options)
        self.add_widget(title_icon)

    def show_font_options(self, instance):
        # Create a Spinner for font selection
        font_spinner = Spinner(text='Roboto', values=('Roboto', 'Arial', 'Times New Roman'))
        
        # Create a Popup for font options
        font_popup = Popup(title="Select Font", content=font_spinner, size_hint=(0.4, 0.4))
        font_spinner.bind(text=font_popup.dismiss)  # Dismiss the popup when a font is selected
        font_popup.open()
    ...
    def show_color_picker(self, instance):
        ...
        # Create a Popup for color picker
        color_popup = Popup(title="Pick a Color", content=color_picker, size_hint=(0.8, 0.8))
        color_picker.bind(color=color_popup.dismiss)  # Dismiss the popup when a color is selected
        color_popup.open()

    def show_title_options(self, instance):
        # Create TextInputs for title customization
        title_input = TextInput(hint_text="Enter title")
        subtitle_input = TextInput(hint_text="Enter subtitle")
        
        # Create a layout for the popup content
        layout = BoxLayout(orientation='vertical')
        layout.add_widget(title_input)
        layout.add_widget(subtitle_input)
        
        # Create a Popup for title options
        title_popup = Popup(title="Customize Title", content=layout, size_hint=(0.6, 0.6))
        title_input.bind(text=title_popup.dismiss)  # Dismiss the popup when title is entered
        subtitle_input.bind(text=title_popup.dismiss)  # Dismiss the popup when subtitle is entered
        title_popup.open()
...
class CourseCreation(BoxLayout):
    def __init__(self, preview_window, **kwargs):
        ...
        self.text_customization_toolbar = TextCustomizationToolbar()

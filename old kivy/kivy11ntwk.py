...
class PDFImporter(BoxLayout):
    ...
    def browse_pdf(self, instance):
        ...
        self.file_chooser.bind(on_submit=self.load_pdf)
        self.popup = Popup(title="Choose a PDF", content=self.file_chooser, size_hint=(0.9, 0.9))
        self.popup.open()
...
class YouTubeInserter(BoxLayout):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.orientation = 'vertical'
        
        self.link_input = TextInput(hint_text='Enter YouTube link', size_hint_y=0.2)
        self.add_widget(self.link_input)
        
        insert_button = Button(text="Insert Video", size_hint_y=0.2)
        insert_button.bind(on_press=self.insert_video)
        self.add_widget(insert_button)

    def insert_video(self, instance):
        link = self.link_input.text
        # Use webbrowser to open the YouTube video in the device's browser
        webbrowser.open(link)  # <-- Replace WebView with this
...
class CourseCreation(BoxLayout):
    def __init__(self, preview_window, **kwargs):
        ...
        # YouTube Inserter
        self.youtube_inserter = YouTubeInserter(size_hint_y=0.3)
        self.add_widget(self.youtube_inserter)
...
class LearningPathApp(App):
    def build(self):
        ...
        course_creation = CourseCreation(size_hint_y=0.6)
        course_creation_tab.add_widget(course_creation)

...
class TextSegment:
    ...
    def from_dict(cls, data):
        return cls(data['text'], data['styles'])
        ...
        with open(FILE_PATH, 'r') as file:
            ...
            for course in courses_data:
                if "content" in course:
                    course_content_segments = [TextSegment.from_dict(data) for data in course["content"]]
                    course["content"] = course_content_segments
            courses.extend(courses_data)
    except FileNotFoundError:
        pass  # Handle the exception as needed, e.g., log an error or create a new file
    return courses
...
def save_courses_to_file(courses):
    with open(FILE_PATH, 'w') as file:
        for course in courses:
            if "content" in course:
                course["content"] = [segment.to_dict() for segment in course["content"]]
...
class CustomTextInput(TextInput):
    ...
    def add_segment(self, text, styles):
        segment = TextSegment(text, styles)
        self.segments.append(segment)
        self.insert_text(segment.text)
        self.markup = True

    def get_segments_as_json(self):
        return json.dumps([segment.to_dict() for segment in self.segments])
...
class TextCustomizationToolbar(BoxLayout):
    ...
    def highlight_text(self, instance):
        ...
        if selected_text:
            self.text_input.delete_selection()
            self.text_input.insert_text(f'[bg=yellow]{selected_text}[/bg]')  # Highlight with yellow background
            self.text_input.markup = True
...
class CourseCreation(BoxLayout):
    def __init__(self, preview_window, **kwargs):
        ...
        self.content_input = CustomTextInput(hint_text='Enter course content', multiline=True, size_hint_y=0.4)
    ...
    def save_course(self, instance):
        ...
        course_content_segments = self.content_input.segments  # Get the segments
        course_content = [segment.to_dict() for segment in course_content_segments]  # Convert segments to a format suitable for JSON serialization

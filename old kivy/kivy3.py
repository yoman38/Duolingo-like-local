...
def load_courses_from_file():
    ...
    with open('courses.json', 'r') as file:
        ...
        # Ensure that the loaded courses are a list of dictionaries
...
class CourseCreation(BoxLayout):
    ...
    def save_course(self, instance):
        ...
        color = self.color_picker.color[:3]  # Get RGB values
...
class CourseCanvas(Widget):
    ...
    def draw_courses(self):
        for course in self.courses:
            ...
            with self.canvas:
                Color(*color)
                Ellipse(pos=(x, y), size=(100, 100))  # Increase the size of the bubble
                
                # Render the course title inside the bubble using CoreLabel and Rectangle
                core_label = CoreLabel(text=course['title'], font_size=14)
                core_label.refresh()
                texture = core_label.texture
                texture_size = list(texture.size)
                Rectangle(pos=(x, y), texture=texture, size=texture_size)
    
    def on_touch_down(self, touch):
        for course in self.courses:
            x, y = course.get('x', 0), course.get('y', 0)
            if ((touch.x - x) ** 2 + (touch.y - y) ** 2) <= 25**2:
                # Display course content in a popup
